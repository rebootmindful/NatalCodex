...
function generateSign(params) {
  if (!config.appSecret) {
    throw new Error('XUNHUPAY_APPSECRET is not set');
  }
  const sortedKeys = Object.keys(params).sort();
  const signStr = sortedKeys
    .filter(key => params[key] && key !== 'hash')  // 官方SDK: 过滤所有falsy值
...
async function createPayment(options) {
  ...
  if (!config.appId || !config.appSecret) {
    throw new Error('XUNHUPAY_APPID or XUNHUPAY_APPSECRET is not set');
  }
...
function handleNotify(body) {
  ...
  const timestamp = parseInt(body.time || '0', 10);
  const nowSec = Math.floor(Date.now() / 1000);
  if (!Number.isFinite(timestamp) || Math.abs(nowSec - timestamp) > 300) {
    return { success: false, error: 'Timestamp window exceeded' };
  }
